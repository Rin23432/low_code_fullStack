...
const BFFPerformanceTest: React.FC = () => {
  ...
  const runComparisonTest = async () => {
    ...
    // 并行运行两种测试
    await Promise.all([measurePerformance("SSR"), measurePerformance("CSR")]);
  ...
  const calculateAverage = (
    type: "SSR" | "CSR",
    metric: keyof PerformanceResult
  ) => {
    const typeResults = results.filter((r) => r.type === type);
    if (typeResults.length === 0) return 0;

    const sum = typeResults.reduce(
      (acc, result) => acc + (result[metric] as number),
      0
    );
  ...
  return (
    <>
      <Head>
        <title>BFF + SSR 性能测试 - 低代码平台</title>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {results.map((result, index) => (
                      <tr
                        key={index}
                        className={
                          result.type === "SSR" ? "bg-blue-50" : "bg-green-50"
                        }
                      >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-blue-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div>
                      首次内容绘制:{" "}
                      {calculateAverage("SSR", "firstContentfulPaint").toFixed(
                        2
                      )}
                      ms
                    </div>
                    ...
                    <div>
                      可交互时间:{" "}
                      {calculateAverage("SSR", "timeToInteractive").toFixed(2)}
                      ms
                    </div>
                    <div>
                      数据加载:{" "}
                      {calculateAverage("SSR", "dataLoadTime").toFixed(2)}ms
                    </div>
                    <div>
                      渲染时间:{" "}
                      {calculateAverage("SSR", "renderTime").toFixed(2)}ms
                    </div>
                    <div className="font-medium">
                      总时间: {calculateAverage("SSR", "totalTime").toFixed(2)}
                      ms
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-green-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div>
                      首次内容绘制:{" "}
                      {calculateAverage("CSR", "firstContentfulPaint").toFixed(
                        2
                      )}
                      ms
                    </div>
                    ...
                    <div>
                      可交互时间:{" "}
                      {calculateAverage("CSR", "timeToInteractive").toFixed(2)}
                      ms
                    </div>
                    <div>
                      数据加载:{" "}
                      {calculateAverage("CSR", "dataLoadTime").toFixed(2)}ms
                    </div>
                    <div>
                      渲染时间:{" "}
                      {calculateAverage("CSR", "renderTime").toFixed(2)}ms
                    </div>
                    <div className="font-medium">
                      总时间: {calculateAverage("CSR", "totalTime").toFixed(2)}
                      ms
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                ...
                <div className="text-sm text-yellow-700">
                  {(() => {
                    ...
                    if (improvement > 0) {
                      return `SSR 相比 CSR 性能提升了 ${improvement.toFixed(
                        1
                      )}%，主要优势在于服务端预渲染减少了客户端的计算负担。`;
                    } else {
                      return `CSR 相比 SSR 性能提升了 ${Math.abs(
                        improvement
                      ).toFixed(1)}%，但 SSR 在首屏加载和 SEO 方面有优势。`;
                    }
                  })()}
                </div>
              </div>
...
export default BFFPerformanceTest;
